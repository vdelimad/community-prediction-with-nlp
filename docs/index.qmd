...
$$ 
\begin{align}\begin{aligned} \min_{w} C \sum_{i=1}^n  (-y_i \log(\hat{p}(X_i)) \\\begin{split}  - (1 - y_i) \log(1 - \hat{p}(X_i)))   + \|w\|_1
...
: This table shows performance metrics across all models, indicating that the Neural Network is the top performer, followed by the Random Forest. The performance metrics include Accuracy, Weighted Precision, Weighted Recall, and ROC AUC. {#tbl-experiment-results}
